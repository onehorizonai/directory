...
- The request is a pure code-correctness review with no UI/UX angle — use
  `review-code` instead.
- The UI is a native iOS/Android/desktop app rather than a web interface —
  use `review-native-ui` instead; the checks here (browser rendering, DOM,
  CSS breakpoints) don't apply to a native shell.
