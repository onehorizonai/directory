...
reconstruct it from the artifact and the product — then judge the UI
against that model, not against taste. Do not invent priorities; mark
them as open questions when evidence is missing.
...
12. Does it adapt correctly to relevant viewport, window, or size-class
    changes?
