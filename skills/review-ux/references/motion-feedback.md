...
motion, feedback, recovery, or accessibility **intent** — after the
model exists. Numeric WCAG, accessibility-tree, and HIG-convention
mechanics belong to a platform build-quality audit, not this skill.

## Familiarity first

Before treating a novel interaction as an improvement, ask whether the
surface's platform already has an established pattern for: navigation,
menus, selection, search, settings, dialogs, sheets, toolbars, context
menus, drag and drop, keyboard, back, destructive actions.
...
Not a final paint pass. While walking each important use case, ask
whether the experience **intends** to support:

- keyboard access where the platform has a keyboard
- logical focus order; visible focus
- names and semantics a screen reader could use
- scalable text
- meaning not by color alone
- large enough targets; alternatives to gesture-only actions
- a reduced-motion path; auto-rotating content can be paused
- sticky chrome does not hide the thing that has focus
- authentication does not depend on memorizing or retyping a secret
  when paste or a password manager could work
- zoom / reflow; longer localized strings; RTL where relevant

Flag missing intent (no visible focus, color-only state, gesture-only
action, no recovery) as UX findings. Do not turn this into a numeric
WCAG or accessibility-tree audit.
