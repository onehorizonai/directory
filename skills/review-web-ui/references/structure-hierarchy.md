...
Visual size and hit area need not match. WCAG 2.2 AA pointer-target
floor is **24×24 CSS px** (with documented exceptions). Keep **~44×44
CSS px** as the touch design target for primary/frequent controls. Do
not treat the AA floor as the design size, and do not import native pt/dp
as web pixels.
